...
class TestCalibrateImageOutputs(lsst.utils.tests.TestCase, MockCheckMixin):
    """Test the output data products of calibrateImage task make sense.
    ...
    def setUp(self):
        ...
        self.exposure = self.butler.get("initial_pvi", self.dataId)
        self.catalog = self.butler.get("initial_stars_footprints_detector", self.dataId)

    def testLocalPhotoCalibColumns(self):
        """Check exposure's photometric calibration are consistent with the
        catalog's photoCalib columns.
        """
        ...
        for rowNum in randomRows:
            ...
            localEval = self.exposure.getPhotoCalib().getLocalCalibration(record.getCentroid())
